...
const classifyLicense = ({
  ...
  if (
    ...
    return {
      accepted: false,
      reason: "NonCommercial license is not allowed.",
    };
  }
  ...
  if (
    ...
    return {
      accepted: false,
      reason: "NoDerivatives license is not allowed.",
    };
  }
...
const scoreCandidate = ({
  ...
  if (normalizeText(fileTitle).includes(normalizedQuery)) {
    score += 45;
  }
  ...
  score += Math.min(35, Math.log2(Math.max(1, megaPixels) + 1) * 12);
...
const evaluatePage = ({
  ...
  const licenseResult = classifyLicense({
    licenseShortName,
    usageTerms,
  });
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        Accept: `${expectedMime},image/*;q=0.8,*/*;q=0.5`,
      },
    });
    ...
    if (responseMime && responseMime !== expectedMime) {
      const acceptablePair =
        (responseMime === "image/jpg" && expectedMime === "image/jpeg") ||
        (responseMime === "image/jpeg" && expectedMime === "image/jpg");
...
const readResolutionCache = async (
  ...
  try {
    ...
    const usable = await fileExistsAndUsable(localPath);
...
export const resolveWikimediaVisual = async ({
  ...
  const candidates = await searchWikimediaVisuals({
    query: cleanQuery,
    preferredOrientation,
    limit: SEARCH_LIMIT_DEFAULT,
  });
