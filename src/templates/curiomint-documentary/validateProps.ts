...
import { validationFailure, validationSuccess } from "../../types/validation";

import { validateDocumentaryAssets } from "../../validation/validateDocumentaryAssets";

import { normalizeDocumentarySections } from "./normalizeDocumentarySections";
...
export const validateDocumentaryProps = (
  ...
  const props = rawProps as unknown as DocumentaryProps;
  ...
  if (
    ...
    !isPositiveNumber(props.durationInSeconds)
  ) {
    return validationFailure(
      "props.durationInSeconds must be greater than 0 when provided",
    );
  }

  if (
    props.introDurationInSeconds !== undefined &&
    !isNonNegativeNumber(props.introDurationInSeconds)
  ) {
    return validationFailure("props.introDurationInSeconds cannot be negative");
  }

  if (
    props.chapterIntroDurationInSeconds !== undefined &&
    !isNonNegativeNumber(props.chapterIntroDurationInSeconds)
  ) {
    return validationFailure(
      "props.chapterIntroDurationInSeconds cannot be negative",
    );
  }

  if (
    props.outroDurationInSeconds !== undefined &&
    !isNonNegativeNumber(props.outroDurationInSeconds)
  ) {
    return validationFailure("props.outroDurationInSeconds cannot be negative");
  }
  const hasSections =
  ...
  if (hasChapters) {
    for (
      let chapterIndex = 0;
      chapterIndex < props.chapters!.length;
      chapterIndex++
    ) {
      const chapter = props.chapters![chapterIndex];

      if (!chapter || !isNonEmptyString(chapter.id)) {
        return validationFailure(
  ...
  for (let sectionIndex = 0; sectionIndex < sections.length; sectionIndex++) {
    ...
    if (!isPositiveNumber(section.durationInSeconds)) {
      return validationFailure(
    ...
    for (let mediaIndex = 0; mediaIndex < section.media.length; mediaIndex++) {
      const media = section.media[mediaIndex];

      if (!media || !isNonEmptyString(media.id)) {
        return validationFailure(
      ...
      if (
        ...
        !isPositiveNumber(media.durationInSeconds)
      ) {
        return validationFailure(
      ...
      if (
        ...
        !isNonNegativeNumber(media.startFromSeconds)
      ) {
        return validationFailure(
      ...
      if (
        media.trimEndSeconds !== undefined &&
        !isNonNegativeNumber(media.trimEndSeconds)
      ) {
        return validationFailure(
      ...
      if (
        media.startFromSeconds !== undefined &&
        media.trimEndSeconds !== undefined &&
        media.trimEndSeconds <= media.startFromSeconds
      ) {
        return validationFailure(
      ...
      if (
        media.shortVideoStrategy !== undefined &&
        media.shortVideoStrategy !== "advance" &&
        media.shortVideoStrategy !== "loop"
      ) {
        return validationFailure(
  ...
  const assetValidation = validateDocumentaryAssets(props);

  if (!assetValidation.valid) {
    return validationFailure(assetValidation.message);
  }
